...
public class BigInt extends Number implements Comparable<BigInt>
{
	...
	public BigInt gcdEuklid(BigInt val) {
		BigInt h;	
		BigInt a = this.copy();
		BigInt b = val.copy();
	...
	public BigInt gcdBinary(BigInt a,BigInt b) {
		int k = 0;
	 ...
	 * 
	 */
	private static final long serialVersionUID = -1851617619490426172L;
	...
	*/
	...
	{
		//System.out.println("start"+this+":"+div);
		// -7/-3 = 2, 2*-3 + -1
		// -7/3 = -2, -2*3 + -1
		// 7/-3 = -2, -2*-3 + 1
		// 7/3 = 2, 2*3 + 1
		...
		/*if(div.len>80) {
			BigInteger t = new BigInteger(this.toString()); 
			BigInteger d = new BigInteger(div.toString());
			this.assign(t.remainder(d).toString());
		}else {*/
		int tmp = compareAbsTo(div);
		if(tmp<0) return;
		if(tmp==0){ setToZero(); return; }
		...
		if(len==dig.length) realloc(len+1); //We need an extra slot.
		div(dig, div.dig, len, div.len, q);
		//System.out.println("end"+this+":"+div);
		for(len = div.len; dig[len-1]==0&&len>1; --len);
		//}
	}
	/**
	...
	*/
	...
	public BigInt pow(int i) {
		...
		//if(base.equals(ZERO)&&i==2)throw new IllegalArgumentException();
		BigInt result = ONE.copy();
		while(i>0) {
			if(i%2==1) {
				result.mul(base);
			}
